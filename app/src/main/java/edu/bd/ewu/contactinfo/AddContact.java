...
public class AddContact extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        image.setOnClickListener(v->{
            if(checkAndRequestPermissions(AddContact.this)){
                chooseImage(AddContact.this);
            }
        });
    }
        ...
        builder.setCancelable(false)
                .setPositiveButton(btn1, (dialog, which) -> {
                    if(btn1.equals("Save")) {
                        ...
                        String id = calendar.getTimeInMillis()+"";

                        ContactsDB db = new ContactsDB(getApplicationContext());
                        db.insertInfo(id, name.getText().toString(), email.getText().toString(),
    ...
    public void onRequestPermissionsResult(int requestCode,String[] permissions, int[] grantResults) {
        ...
        if (requestCode == REQUEST_ID_MULTIPLE_PERMISSIONS) {
            if (ContextCompat.checkSelfPermission(AddContact.this,
                    Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                Toast.makeText(getApplicationContext(),
                                "FlagUp Requires Access to Camera.", Toast.LENGTH_SHORT)
                        .show();
            } else if (ContextCompat.checkSelfPermission(AddContact.this,
